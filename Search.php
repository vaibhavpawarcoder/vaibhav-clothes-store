...
<body>
    ...
    <div class="container">
        <div class="row justify-content-center">
            <?php
            $ID_S = $_GET['ids'];
            $chk_query = "SELECT * FROM product where category='$ID_S' or sub_category='$ID_S' or Pro_category='$ID_S' ";
            $chk_show = mysqli_query($con, $chk_query);
            while ($row = mysqli_fetch_assoc($chk_show)) {
                if ($chk_query) {
            ?>
